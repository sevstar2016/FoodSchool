...
def upgrade() -> None:
    ...
    res = conn.execute(sa.text("SELECT id FROM users WHERE email=:e"), {"e": admin_email}).first()
    if not res:
        conn.execute(
            sa.text(
                """
...
def downgrade() -> None:
    conn = op.get_bind()
    # delete admin user
    conn.execute(sa.text("DELETE FROM users WHERE email=:e"), {"e": settings.admin_email})
